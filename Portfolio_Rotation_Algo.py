...
TOTAL_BUYING_POWER = 210_000

def current_et_time():
    return datetime.now(EASTERN).strftime('%H:%M:%S')

def get_buying_power(mode):
    return TOTAL_BUYING_POWER * BUYING_POWER[mode]
...
def submit_orders(ticker_weights, mode):
    buying_power = get_buying_power(mode)
    extended = (mode == "AH")
    order_type = "limit"
    ...
    for symbol, weight in ticker_weights.items():
        try:
            ...
            if market_price <= 0:
                print(f"[⚠️] Skipped {symbol}: Invalid market price (${market_price})")
                continue

            qty = int((buying_power * weight) // market_price)
            if qty <= 0:
                print(f"[⚠️] Skipped {symbol}: qty=0 (not enough buying power)")
                continue
            ...
            print(f"[BUY] {symbol}: {qty} shares (LIMIT @ ${limit_price}) ~${market_price}")
        except Exception as e:
